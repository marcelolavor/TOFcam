#!/usr/bin/env python3
"""
TOFcam Web Viewer - Versão refatorada usando analyzer_lib
Interface web com análise centralizada
"""
        ...
        if hasattr(self, 'server') and hasattr(self.server, 'latest_frame'):
            ...
            if result:
                return {
                    ...
                    'strategic': {
                        'target_yaw_delta': result.strategic_result.get('yaw_delta', 0),
...
def main():
    print("🌐 TOFcam Web Viewer (Biblioteca Centralizada)")
    print("=" * 50)
    ...
    config = AnalysisConfig(
        ...
        save_frames=False,  # Não salvar por padrão no web viewer
        web_format=True    # Gerar base64 para web
    )
    
    try:
        # Inicializar controlador de câmera
        ...
        # Conectar controlador ao servidor
        httpd.camera_controller = controller
        controller.server = httpd
        
        # Inicializar primeira câmera e começar captura
        controller.switch_camera(0)
